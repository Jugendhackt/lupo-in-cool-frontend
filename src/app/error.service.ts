...
export class ErrorService {
  ...
  public validate(database, oldErrors): Array<string> {
    ...
    // 28 & 29
    let abiCount = 0;
    let alreadyWarned29 = false;
    courses.forEach((courseEl) => {
      if (courseEl.AbiturFach >= 1 && courseEl.AbiturFach <= 4) {
        abiCount++;
        if (courseEl.Kursart_Q1 != '' && courseEl.Kursart_Q2 != '' && courseEl.Kursart_Q3 != '' && courseEl.Kursart_Q4 != '') {
          if (!alreadyWarned29) {
            errors.push('Alle Abiturfächer müssen durchgehend belegt werden.');
            alreadyWarned29 = true;
          }
        }
      }
    });
